...
// Commit-reveal scheme disallows oracles to alter transactions, they can only attempt to censor transactions, which they won't do with verifiable random number roles.
// Hash has to be generated by the user, keccak256 of user address, arguments for the next call, a string key(unique pin or password of any length). all this separated by commas
// without spaces. Hash has to be posted by throwHash(), after oracles relay it to other chain, and after user confirms it's the hash he posted, he can then call cross()
// or callAcross() with the arguments and decryption key
...
contract TrustMinimizedCrossChainBridge { 
	...
	event HashCommitted(address indexed ccnt,bytes32 indexed hash);
	...
	function crossBack(address frm, address tkn, uint mnt, address t, bytes memory dt,string memory k) public onlyAggregator {
		...
		require(hashes[frm] == keccak256(frm,tkn,mnt,t,dt,k));
