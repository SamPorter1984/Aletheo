...
contract DatabaseTestnet {
	...
	function _linkAddress(address founder, address worker) internal {
		if (_linkedAddresses[founder] != worker && _takenAddresses[worker] == false && _isFounder(worker) == false && _founders[founder] >= 1e16) {
	...
	//view functions
	function getAddress(address account) public view returns(bool worker, uint payout, uint founderContrib, address linked, bool taken, bool oracle, uint lastEntryBlock) {
		return (_workers[account],_payouts[account],_founders[account],_linkedAddresses[account],_takenAddresses[account],_oracles[account],_blockNumbers[account]);
	}
