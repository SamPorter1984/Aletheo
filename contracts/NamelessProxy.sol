...
// 2. DEADLINE is a block after which it becomes impossible to upgrade the contract. Defined in constructor and here it's ~2 years.
// Maybe not even required for most contracts, but I kept it as an option.
// 3. UPGRADE_BLOCK defines how often the contract can be upgraded. Defined in _setNextLogic() function and the interval here is set
...
contract NamelessProxy {
	...
	bytes32 internal constant GOVERNANCE_SLOT = 0xc12c99d4355e865334a3f44df14a82eaaea63044ab6149d4017e31cd3d2ffe35;
	...
	constructor() {
		...
		require(UPGRADE_BLOCK_SLOT == bytes32(uint256(keccak256('nameless.proxy.upgradeBlock')) - 1));
		require(DEADLINE_SLOT == bytes32(uint256(keccak256('nameless.proxy.deadline')) - 1));
		require(GOVERNANCE_SLOT == bytes32(uint256(keccak256('nameless.proxy.governance')) - 1));
	...
	function _setAdmin(address newAdm) internal {uint gs = _governanceSet();require(gs < 3);assembly {gs := add(gs,1) sstore(ADMIN_SLOT, newAdm) sstore(GOVERNANCE_SLOT, gs)}}
