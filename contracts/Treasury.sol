...
contract Treasury {
	...
	struct Beneficiary {uint88 amount; uint88 lastClaim; bytes nameType;} // name is for transparency, type is like dev or mod, or bug bounty
	...
	function addBeneficiary(address a, uint amount, uint lastClaim, bytes memory nameType) external {
		require(amount<=30e21 && msg.sender == _governance);
		bens[a].amount = uint88(amount);
		if (block.number < 12640000) {lastClaim = 12640000;}
		bens[a].lastClaim = uint88(lastClaim);
